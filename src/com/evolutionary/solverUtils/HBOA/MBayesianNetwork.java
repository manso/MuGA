...
import com.evolutionary.solverUtils.HBOA.graph.DecisionGraph;
import com.evolutionary.solverUtils.HBOA.graph.IGraph;
import com.evolutionary.solverUtils.HBOA.graph.Leaf;
import com.evolutionary.solverUtils.HBOA.graph.Variable;
import com.evolutionary.solverUtils.HBOA.metric.IBayesianMetric;
import java.util.ArrayList;
import java.util.HashSet;

@SuppressWarnings("unchecked")   	 														// Supress warnings for creating an Array of ArrayList.  
class MBayesianNetwork {           	 														// Check: B. Eckel, "Thinking in Java", 4th ed., MindView Inc., 2006, [pp. 759-761]
    ...
    private DecisionGraph[] decisionGraphs;
    private HashSet<Integer>[] parentList;	// Use this to generate the topological ordering and the CP Tables.
    private HashSet<Integer>[] adjacencyList;	// Use this to insure an acyclic BN.
    private HashSet<Integer>[] splitList; 	// Use this to choose only the correct splits.   																										
    ...
    BinaryString template; // template to generate new individuals

    public MBayesianNetwork(IBayesianMetric bayesianMetric, int offspringSize, int maxVertexDegree, BinaryString template) {
        this.bayesianMetric = bayesianMetric;
        this.offspringSize = offspringSize;
        this.maxVertexDegree = maxVertexDegree;
        this.template = template;
        decisionGraphs = new DecisionGraph[template.getSize()];
        parentList = (HashSet<Integer>[]) new HashSet[template.getSize()];
        adjacencyList = (HashSet<Integer>[]) new HashSet[template.getSize()];
        splitList = (HashSet<Integer>[]) new HashSet[template.getSize()];
    }

    public DecisionGraph[] getDecisionGraphs() {
        return this.decisionGraphs;
    }

    public DecisionGraph getDecisionGraph(int i) {
        return this.decisionGraphs[i];
    }

    public int getBestDecisionGraphPos() {
        return this.bestDecisionGraphPos;
    }

    public double getBestScoreGain() {
        return this.bestScoreGain;
    }

    public void setBestDecisionGraphPos(int i) {
        this.bestDecisionGraphPos = i;
    }

    public void setBestScore(double bestScoreGain) {
        this.bestScoreGain = bestScoreGain;
    }
    ...
    //
    //////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    public void generateModel(SimplePopulation selectedSet) {
        this.generateDecisionGraphs(selectedSet);
        this.topologicalSort();
    }

    private void generateDecisionGraphs(SimplePopulation selectedSet) {
    ...
    private void initializeBN(SimplePopulation selectedSet) {
        int NS = selectedSet.getNumberOfIndividuals();
        this.bestScoreGain = Double.NEGATIVE_INFINITY;
        Solution[] individuals = selectedSet.getIndividuals();
        for (int i = 0; i < template.getSize(); i++) {									// Refresh the Bayesian Network structure.
            parentList[i] = new HashSet<Integer>(template.getSize());				// Initial Capacity = stringSize; 	Default Load Factor = 0.75    
            adjacencyList[i] = new HashSet<Integer>(template.getSize());				// Initial Capacity = stringSize; 	Default Load Factor = 0.75   
            splitList[i] = new HashSet<Integer>(2 * template.getSize());			// Initial Capacity = 2*stringSize; Default Load Factor = 0.75        			 
            for (int n = 0; n < template.getSize(); n++) {
                if (n != i) {
                    splitList[i].add(n);
                }
            }
            selectedSet.computeUnivariateFrequencies();
            int mOne = selectedSet.getUniFrequencies(i);					// Construct the initial single leaf decision graphs.
            int mZero = NS - mOne;
            Leaf newLeaf = new Leaf(0, -1, mZero, mOne, template.getSize());	 				// There is only a single leaf per variable, at depth 0, with side -1.		
            if (mZero > 0 && mOne > 0) {						 				// If mZero = 0 or mOne = 0 there is no need to try any split with this leaf.				
                for (int j = 0; j < NS; j++) {
                    boolean alleleJI = individuals[j].getAllele(i);				// Value of Xi in individual j.
                    for (int s : splitList[i]) {
                        boolean alleleS = individuals[j].getAllele(s);			// Value of Xs in individual j.
                        if (alleleJI == false) {
                            if (alleleS == false) {
    ...
    private void performBestSplit(int i, int j, int k) {
        ...
        HashSet<Integer> descendants = new HashSet<Integer>(template.getSize());		// Initial Capacity = stringSize; Default Load Factor = 0.75
        HashSet<Integer> ascendants = new HashSet<Integer>(template.getSize());		// Initial Capacity = stringSize; Default Load Factor = 0.75
        ...
        decisionGraphs[i].splitBestLeaf(j, k, template.getSize());					 			// Effectively perform the best split.
    }																		// END: performBestSplit(...)

    // Compute scores for new leaf0 and leaf1, and add them to scoreList.
    private void computeNewLeafScores(SimplePopulation selectedSet, int i, int j) {
        int NS = selectedSet.getSize();
        for (int a = 0; a < NS; a++) {
            BinaryString individual = (BinaryString) selectedSet.getIndividual(a);
            ...
            while (!(iterator instanceof Leaf)) {								// Iterator is a variable because we are still traversing the DG.
                ...
                boolean alleleX = individual.getAllele(x);
                if (alleleX == false) {
            ...
            if (itrPosition == j || itrPosition == j + 1) {						// We've reached one of the new leafs.
                ...
                if (mZero > 0 && mOne > 0) {		   							// It's still "interesting" to split.
                    boolean alleleI = individual.getAllele(i);					// Value of Xi in individual a.
                    for (int split : splitList[i]) {
                        boolean alleleS = individual.getAllele(split); 		// Value of Xsplit in individual a.
                        if (alleleI == false) {
                            if (alleleS == false) {
    ...
    private BinaryString generateInstance(BinaryString template) {
        BinaryString ind = (BinaryString) template.getClone();
        boolean[] indiv = ind.getBitsGenome();
    ...
    public void generatePopulation(SimplePopulation pop) {
        pop.clear();
        for (int i = 0; i < pop.maximumSize; i++) {
            pop.addIndividual(generateInstance(template));
        }
    }
