 ...
 * @author zulu
 */
public class TournamentNoRepositionTest {

    GeneticOperator templateOperator = new TournamentNoReposition();
    ...
    private class ThreadPopulation extends Thread {
        ...
        public void run() {
            try {
                ...
                int ind = (int) (pop.getSize() * 0.5);
                op.setParameters("0.5");
                for (int i = 0; i < runs; i++) {
                    //50% 
                    SimplePopulation pop1 = op.execute(pop);
                    assertTrue(ind == pop1.getNumberOfIndividuals());
                    //50%
                    SimplePopulation pop2 = op.execute(pop);
                    assertTrue(ind == pop1.getNumberOfIndividuals());
                    //join
                    pop2.addPopulation(pop1);
//                    System.out.println(pop2.getClass().getSimpleName() + " " + ind*2 + " - " + pop2.getNumberOfIndividuals());
                    assertTrue(ind * 2 == pop2.getNumberOfIndividuals());
                    individual = pop2.getBest();
                }
